...
fn igdb_id_keyed_layout_is_stable() {
	assert_eq!(
		...
		"playmatch:cache:v2:igdb:age_rating:42",
	);
	assert_eq!(
		provider_cache_key("igdb", "company", "1"),
		"playmatch:cache:v2:igdb:company:1",
	);
	assert_eq!(
		provider_cache_key("igdb", "platform_version_release_date", "9999"),
		"playmatch:cache:v2:igdb:platform_version_release_date:9999",
	);
}
...
fn igdb_compound_segments_preserve_embedded_colons() {
	...
	assert_eq!(
		...
		"playmatch:cache:v2:igdb:game:slug:abcdef0123",
	);
	assert_eq!(
		provider_cache_key("igdb", "game:search", "deadbeef"),
		"playmatch:cache:v2:igdb:game:search:deadbeef",
	);
}

#[test]
fn provider_segment_appears_after_version() {
	assert!(provider_cache_key("foo", "game", "1").starts_with("playmatch:cache:v2:foo:game:"));
}
