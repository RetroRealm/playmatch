//! Frozen OpenAPI document for the v1 surface. The per-document server URL
//! supplies the `/api/v1` prefix; handlers carry relative paths so the same
//! handler renders correctly under both version documents.
...
use service::model::{
	...
	GameAndRelationMatchResult, GameAndRelationsResult, GameFileMatchSearch, GameMatchType,
	GameMetadataMatchResult, GameMetadataResponse, GameNameSearchResult, ManualMatchMode,
	MetadataMatchType, MetadataProvider, PlatformMetadataResponse, PlaymatchCompany,
...
#[openapi(
	...
	servers(
		(url = "/api/v1", description = "Version 1 (default)")
	),
	paths(
		health,
		ready,
		...
		get_all_platforms,
		get_platform_by_id,
		get_all_signature_groups,
		get_signature_group_by_id,
		get_sgdb_game_by_id,
